...
$("#boxplotduration").click(function() {
    ...
    if(date == 'All_dates' && truck_id == 'All'){
        query_sql = "SELECT codigo, duracion FROM gps_stats_v7_3 order by codigo, fecha"
        title = 'Total logistic time per truck for selected time period';
    }

    // Range of dates - all trucks
    if(truck_id == 'All' && date != 'All_dates'){
        query_sql = "SELECT codigo, duracion FROM gps_stats_v7_3 WHERE fecha BETWEEN '"+start_date+"' AND '"+end_date+"' order by codigo, fecha"
        title = 'Total logistic time per truck for selected time period';
    }
    ...
    if(truck_id != 'All' && date != 'All_dates' && global_chart == true){
        ...
        title = 'Total logistic time of truck '+truck_id;
    }

    // Range of dates - One or more trucks
    if(truck_id != 'All' && date != 'All_dates' && global_chart == false){
        query_sql = "SELECT codigo, duracion FROM gps_stats_v7_3 WHERE "+many_trucks+" AND " +
    ...
    sql_boxplot.execute(query_sql)
        .done(function(data) {
            ...
            // Get data for chart
                ...
                if(truck_id == 'All'){
                    // Process data
                    boxplotduration_all(title);
                }

                if(truck_id != 'All' && global_chart == true){
                    // Process data
                    boxplotduration_one(title);
                }

                if(truck_id != 'All' && global_chart == false){
                    // Process data
                    boxplotduration_many(title);
                }
...
function boxplotduration_all(title){
    ...
    for(var i = 0; i < trucks_codes.length; i++){
        ...
        if((i + 1) == trucks_codes.length){
            ...
            var serie_n = { name: 'Duration', data: array_boxes };
            series.push(serie_n);
            //console.log(series);

            graph_duration(categories, series, title, median_average);
        }
    }
}
...
function boxplotduration_one(title){
    ...
    for(var i = 0; i < trucks_array.length; i++) {
        ...
        if (arr.length < 5) {
            ...
            // Insuficient Data
            alert("The truck "+trucks_array[i]+' has insufficient data to graph');
...
function boxplotduration_many(title){
 ...
 * Graph
 *********************************************************************************************************************/
function graph_duration(categories, series, title, median){
    $('#chart').highcharts({
